...
import com.example.grolapp2.fragment.*
import com.example.grolapp2.pager.CustomPagerAdapter
...
class HomeActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    val preferenceHelper = lazy { PreferenceHelper(this.applicationContext) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.beginTransaction().replace(R.id.container, ItemListFragment()).commit()
    ...
    private fun initAdapter() {
        ...
        navigationPagerAdapter.addFragment(KauflandFragment())
        navigationPagerAdapter.addFragment(SparFragment())
        navigationPagerAdapter.addFragment(LidlFragment())
        navigationPagerAdapter.addFragment(KonzumFragment())
    ...
    private fun initListeners() {
        ...
        bottomNavigation.setOnNavigationItemSelectedListener { item ->
            when (item.itemId) {
                R.id.action_kauf -> viewPager.currentItem = 0
                R.id.action_spar -> viewPager.currentItem = 1
                R.id.action_lidl -> viewPager.currentItem = 2
                R.id.action_konz -> viewPager.currentItem = 3
            }
            true
        }
    }
    ...
    private fun floatingButton() {
        ...
        fab.setOnClickListener {
            val intent = Intent(this, ItemInputActivity:: class.java)
    ...
    private fun addGeofence(){
        ...
        geoClient?.addGeofences(seekGeofencing(), geofenceIntent)?.run {
            addOnSuccessListener {
                Toast.makeText(this@HomeActivity, "Geofences added", Toast.LENGTH_SHORT).cancel()
            }
            addOnFailureListener {
                Toast.makeText(this@HomeActivity, "Failed to add geofences", Toast.LENGTH_SHORT).show()
    ...
    private fun removeGeofence(){
        geoClient?.removeGeofences(geofenceIntent)?.run {
            addOnSuccessListener {
                Toast.makeText(this@HomeActivity, "Geofences removed", Toast.LENGTH_SHORT).cancel()

            }
            addOnFailureListener {
                Toast.makeText(this@HomeActivity, "Failed to remove geofences", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun validateGadgetAreaInitiateGeofence(resolve: Boolean = true) {
        ...
        locationResponses.addOnFailureListener { exception ->
            if (exception is ResolvableApiException && resolve) {
                ...
                Toast.makeText(this, "Molimo, uključite lokacijske usluge.", Toast.LENGTH_SHORT).show()
            }
        }
